        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-delete-label-{{ $value }}">{{ __('LivewireDatatableMetronic::datatable.modal_delete_title', ['id' => $value]) }}</h5>
                ...
                <div class="modal-body">
                    <p>
                        {{ __('LivewireDatatableMetronic::datatable.modal_delete_info') }}
                    </p>
                    <p class="d-flex justify-content-center">
                        <span x-show="working">@include('datatables::icons.loading') {{ __('LivewireDatatableMetronic::datatable.deleting') }}</span>
                    </p>
                </div>
                <div class="modal-footer">
                    <button x-bind:disabled="working" type="button" class="btn btn-secondary" data-dismiss="modal">
                        {{ __('LivewireDatatableMetronic::datatable.no') }}
                    </button>
                    <span x-on:click="working = !working">
                        <button wire:click="delete({{ $value }})" type="button" class="btn btn-danger" data-dismiss="modal">
                            {{ __('LivewireDatatableMetronic::datatable.yes') }}
                        </button>
                    </span>
                </div>
            </div>
